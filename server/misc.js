...
db = new sqlite3.Database("./server/resumeai.db", err => {
  if (err) {
...
if (!cfpages) {    } 
else{  console.log('server:misc:CF_PAGES' )    }

function queryDb(sql, params = [], env={}) {  
  db = env.cfdb || db;
  key = env.cfkey || key;
  return new Promise((resolve, reject) => { 
    // console.log('queryDb:sql', sql, params)
...
// Calls Worker JS
const { Worker } = require("worker_threads");function executeWorkerFunction(fnName, data) {
  return new Promise((resolve, reject) => {
